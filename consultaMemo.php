...
if ($tipo_consulta == 2) {
    $edificio = 'por_sala';
    $sala = $data->sala;
    $sql = "SELECT * FROM reservas_entry WHERE start_time >= ${desde} AND start_time <= ${hasta} AND room_id = ${sala}";
    ...
    // caso particular edificio Rondeau y Casa de la Cultura, el informe se genera por días y no por sala
    if ($edificio == 'rondeau' || $edificio == 'casa_cultura') {
        ($edificio == 'casa_cultura') ?
            $sql = "SELECT * FROM reservas_entry WHERE start_time >= ${desde} AND start_time <= ${hasta} AND ( room_id LIKE '15' OR room_id LIKE '49')" :
            $sql = "SELECT * FROM reservas_entry WHERE start_time >= ${desde} AND start_time <= ${hasta} AND ( room_id LIKE '36' OR room_id LIKE '51' OR room_id LIKE '39' OR room_id LIKE '40' OR room_id LIKE '48')";
