...
require "functions_autorizaciones.inc";
...
if (
  empty($returl) || ($returl_base[0] == "edit_entry.php") || ($returl_base[0] == "edit_entry_handler.php")
...
$starttime = mktime(
  intval($start_seconds / 3600),
  intval(($start_seconds % 3600) / 60),
  0,
  $start_month,
  $start_day,
  $start_year
...
$endtime = mktime(
  intval($end_seconds / 3600),
  intval(($end_seconds % 3600) / 60),
  0,
  $end_month,
  $end_day,
  $end_year
...
// Ahora MRBS creará la reserva con estos valores ya inyectados
$result = mrbsMakeBookings($bookings, $this_id, $just_check, $skip, $original_room_id, $need_to_send_mail, $edit_type);

// --- INICIO DE LA LÓGICA DE AUTORIZACIONES ---
// Verificamos que el resultado sea exitoso y que no estemos solo chequeando ($just_check)
if ($result['valid_booking'] && !$just_check) {
  ...
  foreach ($bookings as $booking) {
    ...
    // Guardamos los booleanos para los checkboxes
    $booking['vigilancia'] = $datos['vigilancia'];
    $booking['mayordomia'] = $datos['mayordomia'];
    $booking['limpieza'] = $datos['limpieza'];

    // Guardamos la cantidad de horas calculadas
    $booking['horas_vigilancia'] = $datos['h_vigilancia'];
    $booking['horas_mayordomia'] = $datos['h_mayordomia'];
    $booking['horas_limpieza'] = $datos['h_limpieza'];

    $booking['autorizar'] = $datos['req_auth'];
    $booking['estado_autorizacion'] = ($datos['req_auth']) ? 'enviar' : 'autorizado';
    insertar_autorizacion_individual($booking);
  }
}
